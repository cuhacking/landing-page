...
export const EmailSubscriptionForm = () => {
  ...
  return (
    <div className="flex mt-5 w-full">
      <input type="text" placeholder="Email" className="input input-bordered mr-5 rounded-full w-full " onChange={(e) => setValue(e.currentTarget.value)}
        onKeyUp={(e) => {if (e.key === "Enter") submitEmail()}}/>
      <button className="btn btn-secondary rounded-full" onClick={() => {submitEmail()}}>SUBSCRIBE</button>
    </div>
  );
};
...
export const SponsorEmailSubscriptionForm = () => {
  ...
  return (
    <div className="flex">
      <input type="text" placeholder="Email" className="input input-bordered mr-5 rounded-full w-full " onChange={(e) => setValue(e.currentTarget.value)}
        onKeyUp={(e) => {if (e.key === "Enter") submitEmail()}}/>
      <button className="btn btn-outline btn-secondary rounded-full" onClick={() => {submitEmail()}}>SIGN UP</button>
    </div>
  );
};
